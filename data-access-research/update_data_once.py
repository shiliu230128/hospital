...
def parse_wechat_entries() -> dict[str, Any]:
    ...
    for text in texts.values():
        for hid in re.findall(r"^\s+([A-Za-z0-9_]+):\s*\[", text, re.M):
            hospital_ids.add(hid)
    ...
    return {
        ...
        "status": "pass" if not bad_payloads and not missing_local_assets else "needs_attention",
    }
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "wechatEntryValidation": parse_wechat_entries(),
