...
def main() -> None:
    ...
    parser.add_argument("--city", choices=["北京", "上海", "深圳", "杭州"], default=None,
                        help="只抓取指定城市（默认抓全部）")
    ...
    hospitals = parse_hospitals(registry_file) if not args.city or args.city == "北京" else []
    if regional_registry_file:
        regional = parse_regional_hospitals(regional_registry_file)
        if args.city:
            regional = [h for h in regional if h.get("city") == args.city]
        hospitals.extend(regional)
