...
const DEPARTMENTS = [
  ...
  },
  {
    ...
    keywords: ["儿", "小儿", "儿童", "新生儿", "发热", "保健", "白血病"],
  },
  {
    id: "internal",
    label: "内科",
    ...
    keywords: ["内科", "呼吸", "消化", "心内", "肾内", "内分泌", "血液", "风湿", "神经内"],
  },
  {
    id: "surgery",
    label: "外科",
    symbol: "外",
    aliases: ["甲状腺", "胆囊", "泌尿", "头部外伤"],
    keywords: ["外科", "普外", "泌尿", "胸外", "神经外", "肝胆", "骨外"],
  },
  {
    id: "obgyn",
    label: "妇产",
...
function getSchedulesForHospital(hospitalId) {
  return state.schedules.filter((schedule) => {
    ...
    if (state.selectedSubDepartment !== "all" && schedule.subDepartment !== state.selectedSubDepartment) return false;
    ...
    if (state.query) {
      ...
      if (!queryFields.some((field) => matchesText(field, state.query))) {
        return false;
      }
    }
