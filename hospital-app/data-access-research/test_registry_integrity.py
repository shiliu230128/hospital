...
PROTOTYPE_DATA_FILE = FRONTEND_DIR / "prototype-data.js"
UPDATE_REPORT_FILE = SCRIPT_DIR / "update-report.json"
...
def test_prototype_data_refresh_timestamp() -> int:
    text = read_text(PROTOTYPE_DATA_FILE)
    match = re.search(r"window\.PROTOTYPE_RAW_DATA\s*=\s*(\{.*\});\s*$", text, re.S)
    if not match:
        print(f"[FAIL] Unable to parse PROTOTYPE_RAW_DATA from {PROTOTYPE_DATA_FILE.name}")
        return 1

    payload = json.loads(match.group(1))
    report = json.loads(read_text(UPDATE_REPORT_FILE))
    generated_at = payload.get("generatedAt")
    expected_at = report.get("generatedAt")
    source = payload.get("source")
    required_keys = ("bch", "puh3", "pumch")

    if not generated_at or not expected_at:
        print(f"[FAIL] {PROTOTYPE_DATA_FILE.name} or update-report.json is missing generatedAt")
        return 1

    if generated_at != expected_at:
        print(f"[FAIL] {PROTOTYPE_DATA_FILE.name} generatedAt is {generated_at!r}, expected {expected_at!r}")
        return 1

    if source != "data-access-research/update_data_once.py":
        print(f"[FAIL] {PROTOTYPE_DATA_FILE.name} source is {source!r}, expected 'data-access-research/update_data_once.py'")
        return 1

    missing_keys = [key for key in required_keys if key not in payload]
    if missing_keys:
        print(f"[FAIL] {PROTOTYPE_DATA_FILE.name} is missing embedded sample keys: {missing_keys}")
        return 1
    ...
    for key in required_keys:
        ...
        if record_count is None:
            record_count = len(item.get("records") or item.get("sampleRows") or [])
        if record_count <= 0:
            print(f"[FAIL] {PROTOTYPE_DATA_FILE.name} embedded sample {key} is empty")
            return 1

    print(f"[PASS] {PROTOTYPE_DATA_FILE.name} matches the latest refresh timestamp")
    return 0
...
def main() -> int:
    ...
    tests = [
        ...
        ("Prototype data refresh timestamp", test_prototype_data_refresh_timestamp),
    ]
