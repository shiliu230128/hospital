...
def build_prototype_data_payload(report: dict[str, Any]) -> dict[str, Any]:
    sample_files = {
        "bch": DATA_DIR / "bch_schedule_sample.json",
        "puh3": DATA_DIR / "puh3_pediatrics_schedule_sample.json",
        "pumch": DATA_DIR / "pumch_dockervisit_sample_excerpt.json",
    }
    payload = {
        "generatedAt": report.get("generatedAt"),
        "source": "data-access-research/update_data_once.py",
    }
...
def write_prototype_data_asset(report: dict[str, Any]) -> dict[str, Any]:
    payload = build_prototype_data_payload(report)
    PROTOTYPE_DATA_JS.write_text(
        "window.PROTOTYPE_RAW_DATA = " + json.dumps(payload, ensure_ascii=False, indent=2) + ";\n",
        encoding="utf-8",
    )
    return {
        "prototypeDataPath": str(PROTOTYPE_DATA_JS.relative_to(ROOT)),
        "prototypeDataGeneratedAt": report.get("generatedAt"),
        "prototypeDataPolicy": "Embedded schedule samples for static hosting and file:// use; refreshed with the audit report.",
    }
...
def main() -> int:
    ...
    report["frontendRefreshAssets"] = {
        **write_latest_report_assets(report),
        **write_prototype_data_asset(report),
    }
